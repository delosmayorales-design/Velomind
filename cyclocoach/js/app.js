...
document.addEventListener('DOMContentLoaded', () => {
  ...
  mobileStyle.innerHTML = `
    @media (max-width: 768px) {
      ...
      * { box-sizing: border-box !important; }
      ...
      body.light-theme .mobile-menu-sheet { background: rgba(245,246,250,0.97); border-color: rgba(0,0,0,0.07); }
      ...
      body.light-theme .sheet-handle { background: rgba(0,0,0,0.18); }

      .sheet-profile {
        margin: 0 16px 20px; padding: 16px 18px;
        background: rgba(158,214,43,0.07);
        border: 1px solid rgba(158,214,43,0.18);
        border-radius: 20px;
        display: flex; align-items: center; gap: 14px;
      }
      body.light-theme .sheet-profile { background: rgba(158,214,43,0.08); }

      .sheet-avatar {
        width: 52px; height: 52px;
        background: linear-gradient(135deg, #84B422, #C4EF44);
        border-radius: 50%;
        display: flex; align-items: center; justify-content: center;
        font-weight: 800; font-size: 20px; color: #05070a;
        flex-shrink: 0;
        box-shadow: 0 0 18px rgba(158,214,43,0.4);
      }
      .sheet-user-name { font-size: 15px; font-weight: 700; color: #EEF2FF; line-height: 1.2; }
      .sheet-user-sub  { font-size: 12px; color: rgba(255,255,255,0.45); margin-top: 3px; }
      body.light-theme .sheet-user-name { color: #1a1d26; }
      body.light-theme .sheet-user-sub  { color: rgba(0,0,0,0.45); }
      ...
      body.light-theme .sheet-section-label { color: rgba(0,0,0,0.35); }
      ...
      .sheet-nav-item {
        ...
        font-size: 15px; font-weight: 500; font-family: 'Space Grotesk', sans-serif;
        text-decoration: none; border: none; background: none; cursor: pointer; width: auto; text-align: left;
        -webkit-tap-highlight-color: transparent;
        user-select: none; -webkit-user-select: none;
        transition: background 0.15s ease, transform 0.13s cubic-bezier(0.34,1.56,0.64,1);
      }
      .sheet-nav-item:active  { background: rgba(255,255,255,0.07); transform: scale(0.97); }
      .sheet-nav-item.active  { color: var(--primary, #9ED62B); background: rgba(158,214,43,0.08); }
      body.light-theme .sheet-nav-item         { color: rgba(0,0,0,0.75); }
      body.light-theme .sheet-nav-item.active  { color: #7ab822; background: rgba(158,214,43,0.10); }
      ...
      .sheet-nav-item.active .sheet-icon {
        background: rgba(158,214,43,0.14);
        color: var(--primary, #9ED62B);
      }
      body.light-theme .sheet-icon             { background: rgba(0,0,0,0.06); color: rgba(0,0,0,0.4); }
      body.light-theme .sheet-nav-item.active .sheet-icon { background: rgba(158,214,43,0.15); color: #84B422; }

      .sheet-divider { height: 1px; background: rgba(255,255,255,0.06); margin: 10px 16px; }
      body.light-theme .sheet-divider { background: rgba(0,0,0,0.07); }
    }
  ...
  if (document.querySelector('.sidebar')) {
    ...
    // ── Mobile Menu Bottom Sheet ──
    const _closeSheet = () => document.body.classList.remove('mobile-menu-open');

    const sheetOverlay = document.createElement('div');
    sheetOverlay.className = 'mobile-menu-overlay';
    sheetOverlay.addEventListener('click', _closeSheet);
    document.body.appendChild(sheetOverlay);

    const sheet = document.createElement('div');
    sheet.className = 'mobile-menu-sheet';

    const savedProf = (() => { try { return JSON.parse(localStorage.getItem('velomind_profile') || '{}'); } catch { return {}; } })();
    const pName     = (window.AppState?.profile?.name) || savedProf.name || 'Atleta';
    const pFTP      = (window.AppState?.profile?.ftp)  || savedProf.ftp  || '—';
    const pInitials = pName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() || 'VM';

    const sheetItems = [
      { name: 'Inicio',                 icon: 'fa-home',            href: 'dashboard.html' },
      { name: 'Plan de Entrenamiento',  icon: 'fa-calendar-check',  href: 'training-plan.html' },
      { name: 'Nutrición',              icon: 'fa-apple-alt',       href: 'nutrition.html' },
      { name: 'Analíticas',             icon: 'fa-chart-bar',       href: 'analytics.html' },
      { name: 'Rutas & GPS',            icon: 'fa-route',           href: 'rutas.html' },
    ];
    ...
    sheet.innerHTML = `
      ...
      <div class="sheet-profile">
        <div class="sheet-avatar">${pInitials}</div>
        <div>
          <div class="sheet-user-name">${pName}</div>
          <div class="sheet-user-sub">FTP ${pFTP}${pFTP !== '—' ? 'W' : ''}</div>
        </div>
      </div>
      <div class="sheet-section-label">Navegación</div>
      ${sheetItems.map(item => {
        const active = currentPath.includes(item.href) ? 'active' : '';
        return `<a href="${item.href}" class="sheet-nav-item ${active}">
          <span class="sheet-icon"><i class="fas ${item.icon}"></i></span>${item.name}
        </a>`;
      }).join('')}
      <div class="sheet-divider"></div>
      <div class="sheet-section-label">Configuración</div>
      <button class="sheet-nav-item" id="sheet-theme-toggle">
        <span class="sheet-icon"><i class="fas ${themeIcon()}" id="sheet-theme-icon"></i></span>
        <span id="sheet-theme-label">${themeLabel()}</span>
      </button>
      <a href="login.html" class="sheet-nav-item" style="color:rgba(248,113,113,0.85);">
        <span class="sheet-icon" style="color:rgba(248,113,113,0.65);"><i class="fas fa-sign-out-alt"></i></span>
        Cerrar Sesión
      </a>
    `;
    ...
    document.getElementById('sheet-theme-toggle')?.addEventListener('click', () => {
      const next = isLight() ? 'dark' : 'light';
      document.documentElement.setAttribute('data-theme', next);
      document.documentElement.classList.toggle('light-theme', next === 'light');
      localStorage.setItem('velomind_theme', next);
      const tc = document.querySelector('meta[name="theme-color"]');
      if (tc) tc.content = next === 'light' ? '#ffffff' : '#05070a';
    ...
    sheet.querySelectorAll('.sheet-nav-item[href]').forEach(a => a.addEventListener('click', _closeSheet));
