...
const Auth = (() => {
  ...
  function injectUserUI(user) {
    if (!user) return;
    const nameEl   = document.getElementById('athlete-sidebar-name');
    const nameEl2  = document.getElementById('athlete-name');
    const avatarEl = document.getElementById('athlete-avatar');
    const ftpEl    = document.getElementById('athlete-sidebar-ftp');
    const ftpEl2   = document.getElementById('athlete-ftp');
    ...
    if (ftpEl)    ftpEl.textContent    = `FTP: ${user.ftp || '--'} W`;
    if (ftpEl2)   ftpEl2.textContent   = `FTP: ${user.ftp || '--'} W`;
      ...
      logoutWrap.innerHTML = `
        <button id="auth-logout-btn"
          style="width:100%;padding:8px;background:transparent;border:1px solid var(--border);
                 ...
                 transition:all 0.2s; border-color: rgba(255,107,53,0.2);"
  ...
  async function init() {
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => injectUserUI(user));
    } else {
      injectUserUI(user);
    }
    ...
    verifyToken().then(valid => {
      if (valid) {
        ...
        if (refreshed) injectUserUI(refreshed);
      }
    });
