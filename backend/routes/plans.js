...
router.post('/kit', async (req, res) => {
  ...
  if (!design) return res.status(400).json({ error: 'design requerido' });
  ...
  if (error) return res.status(500).json({ error: error.message });
  res.status(201).json({ message: 'Diseño guardado', id: data.id });
});
...
router.put('/kit/:id', async (req, res) => {
  ...
  const { error } = await supabase
    .from('kit_designs')
    .update({ name, design, updated_at: new Date().toISOString() })
    .eq('id', req.params.id)
    .eq('user_id', req.user.id);
  if (error) return res.status(500).json({ error: error.message });
...
router.delete('/kit/:id', async (req, res) => {
  await supabase.from('kit_designs').delete().eq('id', req.params.id).eq('user_id', req.user.id);
